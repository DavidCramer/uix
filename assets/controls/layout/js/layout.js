!(function ($) {
    ...
    $.fn.user_submissionsLayout = function (opts) {
        ...
        return this.each(function () {
            ...
            grid.addClass('_user_submissions_grid_init');

            // setup loading
            grid.html('<div class="user_submissions-loading" style=""></div>');
            ...
            var init_builder = function () {
                ...
                // remove loader
                grid.find('.user_submissions-loading').remove();
            }

            var create_component = function (type, data) {

                var component = $('<div class="user_submissions-component component-' + type + '" data-type="' + type + '" data-label="' + type + '"><div class="user_submissions-component-toolbar"><span class="dashicons dashicons-admin-generic user_submissions-component-edit"></span> ' + type + '<span class="dashicons dashicons-no remove-component right"></span></div></div>'),
            ...
            var remove_column = function (column) {
                ...
                var contents = column.find('.user_submissions-component'),
                ...
                for (var r = 0; r < rows.length; r++) {
                    ...
                    for (var c = 0; c < columns.length; c++) {
                        ...
                        var column = $(columns[c]),
                            component_struct = [],
                            components = column.find('> .column-container > .user_submissions-component');
            ...
            //  add element
            grid.on('click', '.user_submissions-grid-element', function () {
                var clicked = $(this),
            ...
            grid.on('click', '.remove-component', function () {
                var component = $(this).closest('.user_submissions-component');
                if (confirm('Really?')) {
            ...
            grid.on('click', '.remove-row', function () {
                var row = $(this).closest('.row'),
                    contents = row.find('.user_submissions-component');
                if (contents.length) {
            ...
            var component_editor = function () {
                var clicked = $( this ),
                    ...
                    component_modal = $.user_submissionsModal({
                ...
                $(document).trigger('user_submissions.init');
            }
            grid.on('click', '.user_submissions-component-edit', function () {
                var clicked = $(this),
                    component = clicked.closest('.user_submissions-component'),
                    template_ele = $( '#' + grid.prop('id') + '-' + component.data('type') + '-tmpl' ),
                    template = Handlebars.compile( template_ele.html() );

                var component_modal = $.user_submissionsModal({
            ...
            grid.on('click', '.add-component', function () {
                ...
                current_modal = $.user_submissionsModal({
                    modal: 'user_submissions_component',
                    title: 'Insert',
                    width : 550,
                    height: 445,
                    focus: true,
                ...
                $( current_modal.content ).on('click', '.user_submissions-component-trigger', component_editor );
            ...
            function init_rows() {
                ...
                grid.sortable({
                    //handle : '.user_submissions-row-toolbar',
                    //axis: "y",
                    connectWith: ".user_submissions-control-layout",
                    delay: 150,
                    ...
                    helper: function (e, item) {
                        return $('<div class="user_submissions-row-helper" style="width: 155px; height: 30px;"></div>');
                    },
                });

                sort_column_container = grid.find(".column-container").sortable({
                    //handle : '.user_submissions-component-toolbar',
                    delay: 150,
                    forceHelperSize: false,
                    items: '.user_submissions-component',
                    appendTo: "body",
                    connectWith: ".column-container",
                    cursorAt: {top: 35, left: 74},
                    zIndex: 1000000,
                    helper: function (e, item) {
                        return $('<div class="user_submissions-component-helper" style="width: 155px; height: 30px;" data-type="' + item.data('type') + '" data-label="' + item.data('label') + '">' + item.data('label') + '</div>');
                    },
                    update: function () {
                ...
                grid.find('.row').each(function () {
                    ...
                    sort_column = row.find(".column").droppable({
                        accept: '.user_submissions-user_submissionsbuilder-layout-layout_grid-first_row-0-settings-conditions-mini',
                        greedy: true,
            ...
            /// components
            $(".user_submissions-user_submissionsbuilder-layout-layout_grid-first_row-0-settings-conditions-mini").draggable({
                cursor: "move",
                appendTo: "body",
                cursorAt: {top: 15},
                helper: function (event) {
                    var item = $(event.target);
                    return $('<div class="user_submissions-component-helper" data-type="' + item.data('type') + '" data-label="' + item.data('label') + '">' + item.html() + '</div>');
                }
            });
    ...
    $(window).load(function () {
        $(document).on('user_submissions.init', function () {
            $('.user_submissions-control-layout').not('._user_submissions_grid_init').user_submissionsLayout();
        });
    });
