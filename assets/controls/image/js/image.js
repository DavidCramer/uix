(function() {
	...
	jQuery( document ).ready( function( $ ) {
		...
		jQuery( document ).on( 'click', '.uix-image-control-button', function( event ) {
			...
			preview_size = $( this ).data( 'previewSize' );
			var size = $( this ).data( 'size' ) ? $( this ).data( 'size' ) : 'full';
			var type = $( this ).data( 'type' ) ? $( this ).data( 'type' ) : 'id';
			...
			file_frame = wp.media.frames.file_frame = wp.media( {
				...
				multiple: false	// Set to true to allow multiple files to be
				// selected
			} );

			// When an image is selected, run a callback.
			file_frame.on( 'select', function() {
				// We set multiple to false so only get one image from the
				// uploader
